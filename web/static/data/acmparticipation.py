...
with open('acmparticipation.txt', 'w', encoding='utf-8', errors='ignore') as file:
        ...
        for i in range(1, len(df.index.values)):
            ...
            for j in range(0, len(res)):
                ...
                for k in range(0, len(resComplete)):
                    ...
                    if awardStr == cStr:
                        flag = 1
                        output_line = f"{teamName.strip().replace(' ', '')} {to_data[resComplete[k]]['edition']} {universityName} {folder_name.strip().replace(' ', '')}\n"
                        ap_list.append(
                            {
                ...
                if flag == 0:
                    ...
                    for k in range(0, len(resComplete)):
                        ...
                        for n in range(0, to_data[resComplete[k]]['membersNumber']):
                            cStr = cStr + to_data[resComplete[k]]['members'][n]
                        cStr = cStr + to_data[resComplete[k]]['coach']
                        print(cStr)
                    

ap_dict_with_id = {}
for i, team in enumerate(ap_list, start=1):
    ap_dict_with_id[str(i)] = team

output_file = 'acmparticipation.json'
with open(output_file, 'w', encoding='utf-8') as f:
    json.dump(ap_dict_with_id, f, ensure_ascii=False, indent=4)